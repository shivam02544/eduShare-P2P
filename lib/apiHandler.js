...
export function apiHandler(handler, { isProtected = true, schema = null, allowedRoles = null } = {}) {
  return async (req, paramsObj = {}) => {
    ...
    try {
      ...
      // We handle both App Router (context.params) and direct params objects
      const routeParams = paramsObj?.params || (paramsObj && Object.keys(paramsObj).length > 0 ? paramsObj : {});
      ...
      // 2. Parse body for write methods
      if (["POST", "PUT", "PATCH"].includes(req.method)) {
        try {
          const rawBody = await req.json();
          ctx.body = schema ? schema.parse(rawBody) : rawBody;
        } catch (err) {
          ...
          // If body is missing but method is POST/PUT/PATCH, we still want to continue if possible, 
          // or fail gracefully if the inner handler needs it.
          // However, for JSON APIs, a missing body on a POST is often an error.
          logger.debug({ method: req.method, url: req.url }, "Request body empty or invalid JSON");
          ctx.body = {}; 
        }
      }
